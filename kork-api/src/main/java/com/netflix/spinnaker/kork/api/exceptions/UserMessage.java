...
/**
 * An extension point to create exception messages for end-users. Note that the original message on
 * the exception can not be modified as it is immutable - the user message generated here will
 * always be appended to the original message.
 *
 * <p>This can be used to provide more failure-scenario context to end-users.
 */
public interface UserMessage extends SpinnakerExtensionPoint {
   ...
   * @return The string to append to the user message. Note that this will not modify the original
